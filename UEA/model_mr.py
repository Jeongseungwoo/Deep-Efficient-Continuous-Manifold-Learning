...
class ODERGRU(nn.Module):
    def __init__(self, missing_rate, n_class, n_layers, n_units, units, channel, latents, kernels, filters, dilation, bi, device='cpu'):
    ...
    def forward(self, x):
        ...
        for i in range(t):
            ...
            hp = odeint(self.odefunc, torch.cat((h_d.log(), h_l), dim=1), times[i:i + 2], rtol=1e-4, atol=1e-5,
                        method='euler')[1]
            h_d = hp[:, :self.units].tanh().exp()
            h_l = hp[:, self.units:]

            observation = x_l[:, i, :].max(dim=1).values != 0.0

            if observation.any():

                new_h_d = self.rgru_d(x_d[:, i, :], h_d)
                new_h_l = self.rgru_l(x_l[:, i, :], h_l)

                h_d = torch.where(observation.unsqueeze(1), new_h_d, h_d)
                h_l = torch.where(observation.unsqueeze(1), new_h_l, h_l)
...
class Encoder(nn.Module):
    def __init__(self, channel=9, latents=32, kernels="1, 1, 1", filters="8, 12, 16", dilation=1):
        ...
        for i in range(len(filters) - 1):
            self.layers.append(nn.Conv2d(int(filters[i]),
                                         int(filters[i + 1]),
                                         kernel_size=int(kernels[i]),
                                         stride=1,
                                         dilation=dilation))
            self.layers.append(nn.BatchNorm2d(int(filters[i + 1])))
            self.layers.append(nn.LeakyReLU())
        self.layers.append(nn.Conv2d(int(filters[i + 1]), latents, kernel_size=1))

    def forward(self, x):
        b, c, t = x.shape
        observation = x.max(dim=1).values != 0.0
        x.unsqueeze_(1) # [batch, filters, channel, time]
        ...
        for i in range(b):
            ...
            for j in range(t):
                if x[i, :, :, j].max() != 0.00:
                    cov = oas_cov(x[i,:, :, j].transpose(-1, -2))
                    d, l = self.chol_de(cov.unsqueeze(0))
                    x_d_s.append(d.squeeze(0))
                    x_l_s.append(l.squeeze(0))
                else:
                    # x_cov_t.append(torch.diag(torch.ones(x.shape[1], device=x.device)))
                    x_d_s.append(torch.ones(x.shape[1], device=x.device))
                    x_l_s.append(torch.zeros(x.shape[1] * (x.shape[1] - 1) // 2, device=x.device))
            x_d_s = torch.stack(x_d_s, dim=0)
            x_l_s = torch.stack(x_l_s, dim=0)
            # x_cov_t = torch.stack(x_cov_t, dim=0) # seq, c, c
            x_d.append(x_d_s)
            x_l.append(x_l_s)
            # x_cov.append(x_cov_t)
        x_d = torch.stack(x_d, dim=0)
        x_l = torch.stack(x_l, dim=0)
        # x_cov = torch.stack(x_cov, dim=0) # batch, seq, c, c
        # b, s, c, c = x_cov.shape
        # x_d, x_l = self.chol_de(x_cov.reshape(-1, c, c), observation)
        return x_d, x_l
